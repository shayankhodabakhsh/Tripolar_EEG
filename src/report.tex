...
\author{%
  \IEEEauthorblockN{Shayan Kaviani}
  \IEEEauthorblockA{%
...
reactivity (Berger effect), correlation with the disc reference, and VEP
amplitude --- plus FOOOF spectral parameterization and a spectrogram SSIM
...
The short version: the paste TCRE (Ch9/Ch10) is giving the cleanest signal
across all metrics.
...
\begin{table*}[t]
  ...
  \begin{tabular}{llrrrrr}
    ...
    Ch2  & Felt eEEG \#1  &  1.74 & 0.05 & 0.197 &   1.8 & ---          \\
    Ch3  & Felt tEEG \#2  &  0.02 & 0.81 & 0.316 &  48.3 & \textbf{Yes} \\
    Ch4  & Felt eEEG \#2  & -0.51 & 0.16 & 0.389 &   1.8 & ---          \\
    Ch5  & Felt tEEG \#3  &  0.39 & 0.54 & 0.735 &  96.6 & \textbf{Yes} \\
    Ch6  & Felt eEEG \#3  &  2.17 & 0.02 & 0.133 &   4.6 & ---          \\
    Ch7  & Felt tEEG \#4  &  1.32 & 0.69 & 0.869 & 236.8 & \textbf{Yes} \\
    Ch8  & Felt eEEG \#4  & -0.16 & 0.35 & 0.476 &   3.8 & ---          \\
    Ch9  & Paste tEEG \#5 &  1.64 & 0.64 & 0.936 &  13.9 & ---          \\
    Ch10 & Paste eEEG \#5 & -0.75 & 0.60 & 0.968 &   0.8 & ---          \\
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Alpha Reactivity --- Why It's All Below 1}
...
the Berger effect, and what we want to see.
...
directly --- I will fix that next week by normalizing them.
...
\begin{figure}[t]
  ...
  \caption{Eyes-open (green) vs.\ eyes-closed (blue) PSD per channel.
    Y-axis scales differ --- normalized version coming next week.}
  \label{fig:psd}
\end{figure}
...
I ran the FOOOF (specparam) algorithm on each channel to separate the
...
For each TCRE pair, I computed SSIM between the tEEG and eEEG spectrograms.
...
\begin{itemize}
  \item Check electrode impedances before every future session ---
        top priority.
  \item Fix the PSD overlay plots so all channels share a normalized
        y-axis for easier cross-channel comparison.
...
Fig.\,\ref{fig:psd_all} shows the full Welch PSD for all 11 channels after
