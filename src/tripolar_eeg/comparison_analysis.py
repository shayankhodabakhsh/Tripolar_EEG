...
def assign_display_ids(felt_subjects, gel_subjects, key_csv_path=None):
    """Tag each subject with a de-identified code: LF01.., SF01.., G01..

    Codes are assigned alphabetically by raw subject name within each
    sub-cohort. Mutates the ``subj`` dict in place to add a
    ``display_id`` key. If ``key_csv_path`` is given, a CSV mapping is
    written there for internal de-identification (this file should live
    inside the gitignored ``data/`` directory).
    """
    rows = []

    def _bucket(subjects, prefix):
        sorted_pairs = sorted(subjects, key=lambda sr: sr[0]["name"].lower())
        for i, (subj, _) in enumerate(sorted_pairs, start=1):
            code = f"{prefix}{i:02d}"
            subj["display_id"] = code
            rows.append((code, subj["name"], subj.get("basename", ""),
                         subj.get("felt_cohort", "")))
    ...
    _bucket(long_felt,  "LF")
    _bucket(short_felt, "SF")
    _bucket(gel_subjects, "G")

    if key_csv_path:
        with open(key_csv_path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["display_id", "raw_name", "basename", "cohort"])
            for row in rows:
                w.writerow(row)
...
def print_qc_report(subjects_results, label=""):
    ...
    for subj, res in subjects_results:
        ...
        name = subj.get("display_id") or subj["name"][:34]
        n_ch = cfg.n_channels
        ...
        # --- Status logic ---
        # Three-tier QC: PASS (no flags), WARN (one flag), FAIL (>5% clip,
        # SNR<-1 dB, hard data-integrity miss, or multiple WARN flags).
        # Clip 1-2% is below the WARN threshold and reads as PASS — at this
        # level the rail-induced bias is negligible for ratio metrics
        # (alpha SNR, reactivity, SSIM).
        hard_fails = []
        warns = []
        if n_open < 2:
            hard_fails.append(f"only {n_open} open epoch(s)")
        if n_close < 2:
            hard_fails.append(f"only {n_close} close epoch(s)")
        if dur < 60:
            hard_fails.append(f"dur={dur:.0f}s < 60s")
        if not np.isnan(snr_mean) and snr_mean < -1.0:
            hard_fails.append(f"SNR={snr_mean:.1f}dB<-1")
        if clip_pct > 5.0:
            hard_fails.append(f"clip={clip_pct:.1f}%>5%")
        # Soft WARN flags
        if not np.isnan(snr_mean) and -1.0 <= snr_mean < 0:
            warns.append(f"SNR={snr_mean:.1f}dB<0")
        if 2.0 < clip_pct <= 5.0:
            warns.append(f"clip={clip_pct:.1f}%")

        if hard_fails or len(warns) >= 2:
            reasons = hard_fails + (["multiple WARN flags"] if len(warns) >= 2 and not hard_fails else [])
            status = "FAIL  " + "; ".join(reasons + warns if not hard_fails else hard_fails)
        elif warns:
...
def _passes_signal_qc(subj, res, min_alpha_snr_db, min_alpha_reactivity,
                      min_vep_p2p_uv, max_clip_pct=5.0, label=""):
    """
    ...
    Thresholds:
        ...
        min_vep_p2p_uv        - max tEEG VEP peak-to-peak must be >= this (uV);
                                set to 0 to skip this gate.
        max_clip_pct          - mean fraction of int16-rail clipped samples
                                across all channels must be <= this (percent).
                                Set to None to skip.

    Multi-complication rule: a subject fails if it accumulates two or more
    soft WARN flags (clip 2-5%, SNR in [-1, 0) dB, etc.) simultaneously,
    even if no single flag breaches its hard threshold.
    """
    from .eeg_analysis import compute_adc_clipping
    cfg = subj.get("config", FELT_TCRE_CONFIG)
    teeg_idxs = cfg.idx_teeg
    ...
    if min_alpha_snr_db is not None and not np.isnan(mean_snr):
        if mean_snr < min_alpha_snr_db:
            hard_reasons.append(
                f"tEEG Alpha SNR {mean_snr:.1f} dB < {min_alpha_snr_db} dB")
        elif mean_snr < 0:
            warn_flags.append(f"SNR {mean_snr:.1f} dB <0")

    # --- Mean tEEG reactivity ---
    if min_alpha_reactivity is not None:
        ...
        if react_vals:
            mean_react = float(np.mean(react_vals))
    ...
    if min_vep_p2p_uv and min_vep_p2p_uv > 0:
        ...
        if vep_vals:
            max_vep = float(np.max(vep_vals))
            if max_vep < min_vep_p2p_uv:
                hard_reasons.append(
                    f"max tEEG VEP {max_vep:.1f} uV < {min_vep_p2p_uv} uV")

    # --- Clipping gate ---
    if max_clip_pct is not None:
        clip_info = compute_adc_clipping(subj["eeg"])
        clip_pct = float(np.mean(clip_info["clip_fraction"])) * 100.0
        if clip_pct > max_clip_pct:
            hard_reasons.append(f"clip {clip_pct:.1f}% > {max_clip_pct}%")
        elif clip_pct > 2.0:
            warn_flags.append(f"clip {clip_pct:.1f}%")

    # --- Multi-WARN exclusion ---
    if not hard_reasons and len(warn_flags) >= 2:
        hard_reasons.append("multi-WARN: " + "; ".join(warn_flags))
...
def load_all_subjects(felt_dir, gel_dir, felt_names=None,
    ...
    def _try_load(si, data_dir, config, group_key, label):
        ...
        res = analyze_subject(subj, pipeline_variant=pipeline_variant)
    ...
    # --- Felt TCRE ---
    felt_discovered = discover_subjects(felt_dir, recursive=felt_recursive)
    if felt_names:
    ...
    for si in felt_discovered:
        ...
        if subj is not None:
            # Tag long vs short felt based on filename. "long" in the basename
            # (case-insensitive) marks an extended-duration felt session;
            # everything else is short felt.
            is_long = "long" in si["basename"].lower()
            subj["felt_cohort"] = "long" if is_long else "short"
...
def extract_type_metrics(subjects_results, per_subject=True):
    ...
    if per_subject:
        ...
        for subj, res in subjects_results:
            cfg = subj.get("config", FELT_TCRE_CONFIG)
            name = subj.get("display_id") or subj["name"]
...
def _normalize_psd(pxx, f, norm_band=(1, 30)):
    ...
    This makes PSDs from different hardware gain chains (e.g. Gel vs Felt)
