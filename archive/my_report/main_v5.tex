...
A few things I noticed, though I am still getting familiar with how to
interpret these results.
An alpha peak was detected in only 5 of the 11 channels, which I think
partly reflects the clipping problem masking the alpha band on the felt tEEG
channels.
Ch9 (paste tEEG) and Ch10 (paste eEEG) both show clean alpha peaks around
12.7\,Hz and 10.8\,Hz respectively, which seem like the most trustworthy.
Ch10 and Ch11 have poor aperiodic fits ($R^{2} = 0.80$ and $0.02$) --
Ch11's fit is essentially useless, probably because the clipping makes its
spectrum nonstationary. Ch10 has an unusual rising high-frequency tail
that seems to give FOOOF trouble, but I am not entirely sure why.
...
My understanding is that the low correlation is expected because the
Laplacian derivation suppresses the broadly distributed alpha field that
the disc picks up, so it should be capturing more local cortical activity
instead.
If that is right, then the divergence from the disc is actually a good
sign rather than a problem -- but I wanted to confirm that interpretation
with you.
...
The disc and eEEG channels are all under 5\,$\mu$V, which might suggest the
Laplacian is better at resolving focal evoked responses, but I would like
