...
\providecommand{\upmu}{\ensuremath{\mu}}
...
}{\fbox{\parbox{0.7\linewidth}{\centering\vspace{1em}\textit{figure missing --- run \texttt{python scripts/build\_sensitivity\_figure.py}}\vspace{1em}}}}
...
\begin{table}[H]
\caption*{\textbf{Table S1.} Per-class median under each variant, $n$ subjects in parentheses. See \texttt{paper/sensitivity\_abc.md} for the full breakdown across all three endpoints.}
\centering\small
\begin{tabular}{lccc}
\toprule
...
\begin{itemize}
\item Core engine: \texttt{src/tripolar\_eeg/eeg\_analysis.py}; comparison framework: \texttt{src/tripolar\_eeg/comparison\_analysis.py}.
\item Statistics for main text §3.2--§3.6 are produced by \texttt{scripts/build\_paper\_stats.py}; the felt-setup VEP figure and table by \texttt{scripts/build\_felt\_vep\_figure.py}; and the A/B/C sensitivity supplement by \texttt{scripts/build\_sensitivity\_figure.py}.
